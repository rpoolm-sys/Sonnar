...
**Torneio (função `rodarTorneioTerminal`)**: só concorre quem tem edge real comprovado (EWMA de acerto > baseline + margem). Entre aprovados, vence quem tem a janela rápida (últimas 10 calls) mais forte agora. **[CORRIGIDO no Claude Code]** A função recalculava os pacotes dos 3 candidatos direto do estado global (`ewmaTerminais`/`historicoTotal`), mas esse estado já tinha sido atualizado com o giro sendo avaliado antes da checagem — vazamento de dado que inflava a assertividade (EWMA/CUSUM chegou a medir 99.8% de acerto quando vencia). Corrigido lendo a última entrada já calculada de cada callLog (`terminalRadarCallLog`/`mlTerminalCallLog`/`estruturalCallLog`), que já são blindados contra esse vazamento em outro lugar do código. Ver "Achados validados" abaixo pro número real do Torneio combinado. **[SIMPLIFICADO fora do Claude Code]** `renderTerminalRadar()` reescrita pra mostrar só a call oficial do torneio, compacta (1 bloco) — antes eram 4 blocos empilhados (torneio + ranking EWMA + diagnóstico ML + diagnóstico Estrutural). Os 3 motores continuam rodando e acumulando assertividade por baixo, só não aparecem mais como painéis separados. Indicador de confiança combinado:
...
### Modo Compacto (visão mobile)
Botão "📱 Modo Compacto" alterna pra uma visão de 1 linha por rastreador (Trend, Sniper, Trigger, Terminal), pensada pro celular e janelas pequenas de PC — não recalcula nada, só lê o estado que os motores já produzem. Mostra também uma linha com os últimos 10 resultados (mesma cor da matriz principal). Preferência salva em `localStorage` (`mingModoCompacto`), persiste entre sessões. Funções: `alternarModoCompacto()`, `renderModoCompacto()`, `linhaCompacta()`. Validado no celular pelo Ricardo.
...
- Quebra de Número como candidato do Torneio: nulo em ambas as versões testadas — por número individual só funcionava em sessões longas (600+ giros; fraco/negativo em 150-300 giros), versão global (mistura qualquer número numa sequência só) deu 10.0% em todos os tamanhos testados, p~1.0. Não entrou no torneio; fica só como destaque visual dourado (⭐) no painel "Histórico de Quebra" quando a quebra de Número é ≥50 giros — não alimenta nenhuma previsão. Código de suporte (`atualizarCandidatoQuebra()` e variáveis relacionadas) definido mas sem chamada no fluxo principal, dead code intencional.
...
- **[fora do Claude Code] Reset não limpava o Modo Compacto** — `resetarTudo()` não limpava os containers do Modo Compacto (`compactoUltimos`/`compactoQuebraAlerta`/`compactoLinhas`), que ficavam mostrando dado antigo até o próximo giro chegar, mesmo com o resto já resetado por baixo. Corrigido.
...
- Simular o motor em Node exige mockar `document`, `localStorage`, `confirm`, `alert`, `Blob`, `URL` (ver qualquer teste anterior no histórico como modelo). O mock de elemento (`FakeElement`) precisa incluir `.style = {}` e `document` precisa de `querySelectorAll: () => []` — o Modo Compacto usa os dois e quebra o harness (não o app, que roda liso no browser) se faltar.
